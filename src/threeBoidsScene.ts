...
import velocityComputeWGSLRaw from './shaders/velocity-compute.wgsl?raw';
import positionComputeWGSLRaw from './shaders/position-compute.wgsl?raw';
import birdVertexWGSLRaw from './shaders/bird-vertex.wgsl?raw';

// Split WGSL to remove storage declarations for Three.js
const velocityComputeWGSL = velocityComputeWGSLRaw.split('//split-here')[1].trim();
const positionComputeWGSL = positionComputeWGSLRaw.split('//split-here')[1].trim();
const birdVertexWGSL = birdVertexWGSLRaw.split('//split-here')[1].trim();

console.log('velocityComputeWGSL:', velocityComputeWGSL.substring(0, 200));
console.log('positionComputeWGSL:', positionComputeWGSL.substring(0, 200));
console.log('birdVertexWGSL:', birdVertexWGSL.substring(0, 200));
...
export async function threeBoidsInit() {
  // Camera setup
  ...
  const birdVertexShader = wgslFn(birdVertexWGSL, [positionStorage, velocityStorage, phaseStorage]);
  ...
  const velocityComputeShader = wgslFn(velocityComputeWGSL, [positionStorage, velocityStorage]);
  const positionComputeShader = wgslFn(positionComputeWGSL, [positionStorage, velocityStorage, phaseStorage]);
